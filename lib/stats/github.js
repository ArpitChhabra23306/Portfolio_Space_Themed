 ...
 */
async function fetchUserProfile(username, signal) {
 ...
 */
async function fetchTotalStars(username, signal) {
 ...
 */
async function fetchContributionCalendar(username, token, signal) {
 ...
 */
export async function fetchGitHubStats(username, token, signal) {
  const [profile, totalStars, contributionCalendar] = await Promise.all([
    fetchUserProfile(username, signal),
    fetchTotalStars(username, signal),
    fetchContributionCalendar(username, token, signal),
  ]);
